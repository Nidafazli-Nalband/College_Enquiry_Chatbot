ready(()=>{
	...
	// Helper to build headers including token
	...
	uploadForm && uploadForm.addEventListener('submit', async (ev)=>{
		...
		const token = localStorage.getItem('mmec_token')
		try{
			const res = await fetch('/api/admin/upload',{method:'POST', headers: getHeaders(false), body: form})
	...
	btnList && btnList.addEventListener('click', async ()=>{
		...
		const token = localStorage.getItem('mmec_token')
		try{
			const res = await fetch('/api/admin/upload', { headers: getHeaders(false) })
			const j = await res.json()
	...
	btnToggle && btnToggle.addEventListener('click', async ()=>{
		const token = localStorage.getItem('mmec_token')
		try{
			const res = await fetch('/api/admin/toggle_ai', {method:'POST', headers: getHeaders(true)})
			const j = await res.json()
	...
	btnUpdateStudent && btnUpdateStudent.addEventListener('click', async ()=>{
		...
		const token = localStorage.getItem('mmec_token')
		try{
			const res = await fetch('/api/admin/update_student', {method:'POST', headers: getHeaders(true), body: JSON.stringify({email, marks})})
			const j = await res.json()
