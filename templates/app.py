...
def remove_logs_containing(substr):
    """Remove any entries in the global chat logs whose bot_msg contains substr (case-insensitive)."""
    try:
        ...
        new_logs = []
        for entry in logs:
            bot_msg = (entry.get('bot_msg') or '').lower()
            if s in bot_msg:
                # skip this entry (effectively delete it)
                continue
            new_logs.append(entry)
        if len(new_logs) != len(logs):
            save_logs(new_logs)
    except Exception as e:
        print('remove_logs_containing error', e)


def remove_histories_containing(substr):
    """Scan per-user history files and remove any history items whose text contains substr."""
    try:
        s = (substr or '').strip().lower()
        if not s:
            return
        ...
        for fn in os.listdir(HIST_DIR):
            ...
            try:
                ...
                lst = []
            new_lst = [it for it in lst if s not in ((it.get('text') or '').lower())]
            if len(new_lst) != len(lst):
                try:
                    with open(p, 'w', encoding='utf-8') as f:
                        json.dump(new_lst, f, indent=2)
                except Exception as e:
                    print('failed writing history file', p, e)
    except Exception as e:
        print('remove_histories_containing error', e)
...
def api_admin_data():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if success:
                # If an admin deleted a fee structure, also try to remove any
                # chat log / history entries referencing that course type so
                # stale fee messages stop appearing in the UI.
                try:
                    if table == 'fee_structure' and action == 'delete' and course_type:
                        # remove occurrences matching course_type (case-insensitive)
                        remove_logs_containing(course_type)
                        remove_histories_containing(course_type)
                        # also try removing plain course_type variants
                        remove_logs_containing(str(course_type).lower())
                        remove_histories_containing(str(course_type).lower())
