...
# Read from environment variable ADMIN_EMAILS (comma separated) if present; otherwise use default.
ADMIN_EMAILS = [e.strip() for e in os.getenv('ADMIN_EMAILS', 'nidafazlinalband@gmail.com').split(',') if e.strip()]
...
# Legacy users DB (will be migrated and removed after migration completes)
USERS_DB = os.path.join('data', 'users.db')
# `MMEC_DB` stores chat histories, unanswered queries and all app tables (we will consolidate users into this DB)
MMEC_DB = os.path.join('data', 'mmec.db')
# Use mmec.db as the authoritative DB for users and admin data
DB_PATH = MMEC_DB
...
def api_query():
    ...
    def sanitize_bot_msg(msg):
        ...
        for line in lines:
            ...
            line = line.lstrip(':-–— \n').strip()
            # Remove noisy navigation headers like "From https://..." or site chrome
            if re.match(r'^From\s+https?:\/\/', line, re.IGNORECASE):
                continue
            # Remove common site chrome phrases
            if 'skip to content' in line.lower() or 'skip to' in line.lower():
                continue
            # Remove repeated site title fragments
            if re.search(r'mmar?tha mandal|maratha mandal engineering college', line, re.IGNORECASE):
                # skip overly short title-only lines
                if len(line) < 200:
                    continue
        ...
        out = '\n'.join(cleaned)
        # Collapse many whitespace/newlines into paragraphs
        out = re.sub(r'\n{2,}', '\n\n', out)
        out = re.sub(r'[\t\r]+', ' ', out)
        out = out.strip()
        # Truncate very long scraped snippets to a reasonable length (preserve sentence end if possible)
        max_len = 800
        if len(out) > max_len:
            # try to cut at last sentence end before max_len
    ...
    if isinstance(ai_answer, str):
        ...
        # If the AI explicitly indicates it cannot answer (or returns a generic error),
        # record the original user question so admins can review and add a FAQ.
        try:
            low = short.lower()
            incap_phrases = [
                "sorry",
            ...
            if any(p in low for p in incap_phrases):
                try:
                    record_unanswered(message)
                except Exception:
                    pass
...
def api_logs():
    ...
    if request.method == 'GET':
        try:
            ...
            # If the chat logs file is corrupted or unreadable, repair it and return an empty list
            try:
                save_logs([])
            except Exception:
                pass
    ...
    if request.method == 'POST':
        ...
        entry = {
            ...
            # sanitize bot_msg below and still save the log so dashboard shows real query counts
            "bot_msg": bot_msg,
...
def api_admin_reply():
    ...
    if not entry_id or reply is None:
        # Require entry identifier. Allow empty-string replies (treated as valid short replies),
        # but if the caller didn't provide the `reply` key at all, return an error.
    ...
    save_logs(logs)
    # Persist this reply into admin_faqs so future matching queries can be answered automatically.
    try:
        qtext = ''
        # try to extract the original user question from the log entry we just modified
        for entry in logs:
            if entry.get('id') == entry_id or entry.get('ts') == entry_id:
                qtext = entry.get('user_msg') or entry.get('question') or ''
                break
        conn2 = sqlite3.connect(DB_PATH)
        cur2 = conn2.cursor()
        cur2.execute('INSERT INTO admin_faqs (question, answer, keywords, ts) VALUES (?, ?, ?, ?)', (qtext, reply or '', '', datetime.utcnow().isoformat() + 'Z'))
        conn2.commit()
        conn2.close()
    except Exception:
        pass
...
def init_mmec_db():
    ...
    os.makedirs(os.path.dirname(MMEC_DB), exist_ok=True)
    conn = sqlite3.connect(MMEC_DB)
    cur = conn.cursor()
    ...
    ''')
    # users table: consolidate full user account schema here (previously in users.db)
    cur.execute('''
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL,
...
def migrate_users_db_to_mmec():
    """Migrate users from the legacy `data/users.db` into `data/mmec.db`.
    This is idempotent: existing emails in mmec.db will not be duplicated.
    After successful migration, the legacy users.db file will be backed up and removed.
    """
    try:
        if not os.path.exists(USERS_DB):
            return
        # Ensure mmec DB initialized
        init_mmec_db()
        # Connect to legacy and target DBs
        src_conn = sqlite3.connect(USERS_DB)
        src_cur = src_conn.cursor()
        # Attempt to read common columns; be defensive about schema
        try:
            src_cur.execute('SELECT name, email, mobile, password_hash, security_question, answer, created_at, last_login, marks, notes, chat_permission FROM users')
            rows = src_cur.fetchall()
        except Exception:
            # Try alternative column sets (older simple schema)
            try:
                src_cur.execute('SELECT name, email, password_hash FROM users')
                rows = src_cur.fetchall()
                # normalize to full shape with placeholders
                rows = [(r[0] if len(r) > 0 else '', r[1] if len(r) > 1 else '', '', r[2] if len(r) > 2 else '', '', '', None, None, '', '', 1) for r in rows]
            except Exception:
                rows = []
        src_conn.close()
        if not rows:
            return
        dest_conn = sqlite3.connect(MMEC_DB)
        ...
        for r in rows:
            ...
            mobile = r[2] if len(r) > 2 else ''
            password_hash = r[3] if len(r) > 3 else ''
            security_question = r[4] if len(r) > 4 else ''
            answer = r[5] if len(r) > 5 else ''
            created_at = r[6] if len(r) > 6 and r[6] else datetime.utcnow().isoformat() + 'Z'
            last_login = r[7] if len(r) > 7 else None
            marks = r[8] if len(r) > 8 else ''
            notes = r[9] if len(r) > 9 else ''
            chat_permission = int(r[10]) if len(r) > 10 and r[10] is not None else 1
            if not email:
                continue
            # Check existing by email
            try:
                dest_cur.execute('SELECT id FROM users WHERE email = ?', (email,))
                if dest_cur.fetchone():
                    continue
            except Exception:
                pass
            try:
                dest_cur.execute('INSERT INTO users (name, email, mobile, password_hash, security_question, answer, created_at, last_login, marks, notes, chat_permission) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (name, email, mobile, password_hash, security_question, answer, created_at, last_login, marks, notes, chat_permission))
                inserted += 1
            except Exception:
                continue
        dest_conn.commit()
        dest_conn.close()
        # If we inserted something, back up and remove legacy DB
        if inserted > 0:
            try:
                bak = USERS_DB + '.bak'
                if os.path.exists(bak):
                    os.remove(bak)
                os.rename(USERS_DB, bak)
            except Exception:
                pass
    except Exception as e:
        print('User DB migration error', e)
...
def api_admin_delete_student():
    ...
    try:
        # Primary users DB (users.db) contains the registered users
        users_db = os.path.join('data', 'users.db')
        try:
            conn = sqlite3.connect(users_db)
            cur = conn.cursor()
            ...
            # continue even if users.db operations fail
            try:
                conn.close()
            except Exception:
                pass

        # mmec.db (if present) also contains histories/chat tables; remove entries there too
        mmec_db = os.path.join('data', 'mmec.db')
        try:
            if os.path.exists(mmec_db):
                conn2 = sqlite3.connect(mmec_db)
                cur2 = conn2.cursor()
                # delete from histories and chat_history where applicable
                try:
                    cur2.execute('DELETE FROM histories WHERE username = ? OR role = ?', (email, email))
                except Exception:
                    pass
                try:
                    cur2.execute('DELETE FROM chat_history WHERE message LIKE ? OR reply LIKE ?', (f'%{email}%', f'%{email}%'))
                except Exception:
                    pass
                conn2.commit()
                conn2.close()
        ...
        # Remove any active in-memory sessions for this user (so deleted user cannot continue)
        try:
            tokens_to_remove = [t for t, v in list(SESSIONS.items()) if v == email]
            for t in tokens_to_remove:
                SESSIONS.pop(t, None)
        except Exception:
            pass

        # Remove admin_faqs entries that reference this user's email or username to avoid orphaned rules
        try:
            username = email.split('@')[0]
            conn3 = sqlite3.connect(DB_PATH)
            cur3 = conn3.cursor()
            try:
                cur3.execute("DELETE FROM admin_faqs WHERE question LIKE ? OR keywords LIKE ?", (f'%{email}%', f'%{email}%'))
            except Exception:
                pass
            try:
                cur3.execute("DELETE FROM admin_faqs WHERE question LIKE ? OR keywords LIKE ?", (f'%{username}%', f'%{username}%'))
            except Exception:
                pass
            conn3.commit()
            conn3.close()
...
def api_admin_unanswered():
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        # Only return entries that are still unanswered (answered is NULL or 0)
        cur.execute('SELECT id, question, answer, answered, timestamp, answered_at, answered_by FROM unanswered_queries WHERE answered IS NULL OR answered=0 ORDER BY id DESC')
        rows = cur.fetchall()
        conn.close()
        out = []
        for r in rows:
...
def api_admin_answer_unanswered():
    ...
    data = request.get_json() or {}
    qid = data.get('id')
    # Accept empty string as explicit deletion. If caller omitted the 'answer' key
    # entirely, treat it as an empty answer (deletion) for robustness with some clients.
    answer = data.get('answer', '')
    # qid is required.
    if not qid:
        return jsonify({"ok": False, "error": "missing id"}), 400
    try:
        ...
        if answer == '':
            # treat empty answer as a request to remove the unanswered record
            cur.execute('DELETE FROM unanswered_queries WHERE id=?', (qid,))
            if cur.rowcount == 0:
                conn.close()
                return jsonify({"ok": False, "error": "not found"}), 404
        else:
            # Mark unanswered as answered and add to admin_faqs for future automatic replies
            cur.execute('UPDATE unanswered_queries SET answer=?, answered=1, answered_at=?, answered_by=? WHERE id=?', (answer, now, user, qid))
            cur.execute('INSERT INTO admin_faqs (question, answer, keywords, ts) VALUES (?, ?, ?, ?)', (data.get('question') or '', answer, data.get('keywords') or '', now))
        conn.commit()
        conn.close()
        return jsonify({"ok": True})
    except Exception as e:
